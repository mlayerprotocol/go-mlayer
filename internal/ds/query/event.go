...
func GetEventById(id string, modelType entities.EntityModel) (*entities.Event, error) {
	...
	logger.Infof("GETTINGEVENTBYKEY %s", key)
...
func createEvent(event *entities.Event, tx *datastore.Txn, wbParam *badger.WriteBatch) (err error) {
	...
	keys := event.GetDataStoreKeys()
	logger.Debugf("Creating Event: %+v", keys)
	...
	for _, key := range keys {
		...
		if key == event.BlockKey() {
			...
			if txn == nil {
				err = wb.Set(datastore.NewKey(key).Bytes(), []byte{})

			} else {
				err = txn.Put(context.Background(), datastore.NewKey(key), []byte{})
			}
			if err != nil {
				return err
			}
		...
		if key == event.DataKey() {
			...
			if txn == nil {
				logger.Infof("settingDataKey %s", key)
				err = wb.Set(datastore.NewKey(key).Bytes(), eventBytes)

			} else {
				logger.Infof("ADDING EVENT TXN %v", err)
			...
			if txn == nil {
				err = wb.Set(datastore.NewKey(key).Bytes(), utils.UuidToBytes(event.ID))
			} else {
				err = txn.Put(context.Background(), datastore.NewKey(key), utils.UuidToBytes(event.ID))
			}
			if err != nil {
				logger.Infof("ADDING EVENT %v", err)
				return err
			}
		}
...
func UpdateEvent(event *entities.Event, tx *datastore.Txn, wbParam *badger.WriteBatch, create bool) (err error) {
	...
	eventBytes := event.MsgPack()
	if txn == nil {
		err = wb.Set(datastore.NewKey(event.DataKey()).Bytes(), eventBytes)
	} else {
		err = txn.Put(context.Background(), datastore.NewKey(event.DataKey()), eventBytes)
	}
	if err != nil {
		logger.Errorf("PUTEVENTERROR: %v", err)
		return err
	}
...
func GetStateFromEventPath(path *entities.EventPath) (any, error) {
	...
	// logger.Infof("HELOOOOOOS %+v", state)
	err = encoder.MsgPackUnpackStruct(b, state)
	logger.Infof("HELOOOOOOS %+v, Error: %v", state, err)
	return state, err
}
