...
func HandleNewPubSubMessageEvent(event *entities.Event, ctx *context.Context) {
	...
	authEventUpToDate = query.EventExist(&event.AuthEventHash) || (authState.ID == "" && event.AuthEventHash.Hash == "") || (authState.ID != "" && authState.EventHash == event.AuthEventHash.Hash)
	...
	if string(event.Validator) != (*cfg).NetworkPublicKey {
		...
		for _, dep := range *dependent {
			go HandleNewPubSubMessageEvent(&dep.Event, ctx)
		}
	}
