...
func SaveUnprocessedAuthorizationPayload(key string, payload *entities.Authorization, ctx *context.Context) error {
	...
	error := unprocessedStore.Set(*ctx, db.Key(key), payload.ToJSON(), false)
	if error != nil {
		return error
	}
